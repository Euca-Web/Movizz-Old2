import type comments from "../model/comments.js";
...
// la fonction renvoie un objet unknown lorsqu'une erreur est renvoyée 
	public selectAll = async (): Promise<comments[] | unknown> => {
	...
	public selectOne = async (data: Partial<comments>): Promise<comments[] | unknown> => {
		const connection = await new MySqlService().connect();
		// console.log(connection);
		...
		try {
			...
			const result = (results as comments[]).shift();
